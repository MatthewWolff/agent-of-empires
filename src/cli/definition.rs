...
pub enum Commands {
    ...
    /// Internal: the built-in diagnostics worker, spawned by the plugin host
    /// as a self-exec worker. Samples memory + agent counts and pushes them to
    /// the home-pane slot over the plugin protocol. Hidden from help.
    #[command(name = "__plugin-diagnostics", hide = true)]
    PluginDiagnostics,
...
pub fn command_name(command: &Commands) -> Option<&'static str> {
    Some(match command {
        ...
        Commands::PluginDiagnostics => return None,
